...
class VerticaHook(DbApiHook):
    ...
    def __init__(self, resourcepool=None,*args, **kwargs):
    ...
    def get_conn(self):
        ...
        conn.cursor().execute('SET SESSION resource_pool={resourcepool}'.format(resourcepool=self.resourcepool if self.resourcepool else 'default'))
        return conn
    ...
    def get_connection_config(conn, driver_type='vertica_python'):
        ...
        if 'ssl' in conn.extra_dejson:
            if driver_type == 'pyodbc':
                # FIXME: does pyodbc support ssl?
                logger.warning('pyodbc does not support ssl.  Ignoring this connection parameter.')
            elif driver_type == 'vertica_python':
                ssl_param = conn.extra_dejson['ssl']
                logger.info('Using extra param ssl: {}'.format(ssl_param))
                if ssl_param and ssl_param not in ('f', 'False', 'false'):
